"use client"

import Header from "@/components/Header"
import axios from "axios"
import { useEffect, useState } from "react"

interface UserProfile {
  display_name?: string
  images?: { url: string }[]
}

export default function TasteAnalysis() {
  const [accessToken, setAccessToken] = useState<string>("")
  const [userProfile, setUserProfile] = useState<UserProfile | null>(null)

  useEffect(() => {
    const storedToken = localStorage.getItem("spotify_access_token")
    if (storedToken) {
      setAccessToken(storedToken)
    } else {
      window.location.href = "/"
    }
  }, [])

  useEffect(() => {
    if (accessToken) {
      fetchUserProfile()
    }
  }, [accessToken])

  const fetchUserProfile = async () => {
    if (!accessToken) return
    try {
      const response = await axios.get("/api/user-profile", {
        headers: {
          Authorization: `Bearer ${accessToken}`,
        },
      })
      setUserProfile(response.data)
    } catch (error) {
      console.error("Error fetching user profile:", error)
    }
  }

  if (!accessToken) {
    return (
      <div className="min-h-screen bg-black flex items-center justify-center">
        <div className="text-center">
          <p className="text-white mb-4">Please log in to view your taste analysis</p>
          <button
            onClick={() => (window.location.href = "/")}
            className="bg-green-500 text-black font-medium py-2 px-4 rounded-full"
          >
            Go to Login
          </button>
        </div>
      </div>
    )
  }

  return (
    <div className="min-h-screen bg-black text-white">
      {/* Animated Background */}
      <div className="fixed inset-0 z-0">
        <div className="absolute inset-0 bg-gradient-to-br from-green-900/20 via-black to-green-900/20"></div>
        <div className="absolute top-0 left-0 w-full h-full bg-[url('/placeholder.svg?height=800&width=800')] bg-repeat opacity-5"></div>
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,transparent_0%,black_70%)]"></div>
      </div>

      <Header accessToken={accessToken} userProfile={userProfile} />

      <main className="relative z-10 py-10 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto">
          <div className="mb-8">
            <h1 className="text-4xl font-bold mb-2">Taste Analysis</h1>
            <p className="text-gray-400">Deep dive into your music preferences and patterns.</p>
          </div>

          {/* Coming Soon Message */}
          <div className="bg-white/5 backdrop-blur-sm rounded-xl border border-white/10 p-10 text-center">
            <div className="w-16 h-16 mx-auto mb-4 rounded-full bg-green-900/30 flex items-center justify-center">
              <svg
                viewBox="0 0 24 24"
                className="w-8 h-8 text-green-500"
                fill="none"
                stroke="currentColor"
                strokeWidth="2"
              >
                <path d="M2 12h5"></path>
                <path d="M9 12h5"></path>
                <path d="M16 12h6"></path>
                <path d="M4 7v10"></path>
                <path d="M11 7v10"></path>
                <path d="M18 7v10"></path>
              </svg>
            </div>
            <h3 className="text-xl font-bold mb-2">Coming Soon</h3>
            <p className="text-gray-400 mb-6 max-w-md mx-auto">
              We're building a comprehensive analysis of your music taste. This feature will be available soon.
            </p>
            <button className="bg-green-600 hover:bg-green-500 text-white font-medium py-2 px-4 rounded-lg transition-colors">
              Get Notified When Ready
            </button>
          </div>

          {/* Preview Section */}
          <div className="mt-12">
            <h2 className="text-2xl font-bold mb-6">Preview</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="bg-white/5 backdrop-blur-sm rounded-xl border border-white/10 p-6">
                <h3 className="text-lg font-medium mb-4">Genre Breakdown</h3>
                <div className="aspect-square bg-gray-900 rounded-lg flex items-center justify-center">
                  <p className="text-gray-500">Genre analysis preview</p>
                </div>
              </div>

              <div className="bg-white/5 backdrop-blur-sm rounded-xl border border-white/10 p-6">
                <h3 className="text-lg font-medium mb-4">Listening Patterns</h3>
                <div className="aspect-square bg-gray-900 rounded-lg flex items-center justify-center">
                  <p className="text-gray-500">Listening patterns preview</p>
                </div>
              </div>

              <div className="bg-white/5 backdrop-blur-sm rounded-xl border border-white/10 p-6">
                <h3 className="text-lg font-medium mb-4">Music Personality</h3>
                <div className="aspect-square bg-gray-900 rounded-lg flex items-center justify-center">
                  <p className="text-gray-500">Personality analysis preview</p>
                </div>
              </div>
            </div>
          </div>
        </div>
      </main>
    </div>
  )
}

